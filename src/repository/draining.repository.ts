...
export class DrainingRepository extends AbstractRepository<Draining> {
    ...
    private GET_DRAINING_ACCEPTED_UNDONE = 'SELECT d.*, slot.name, home.address, home.address_plus, home.phone, home.zip, home.city, user.firstname, user.lastname, draining_request.slot_id, slot.id FROM draining as d INNER JOIN draining_request ON draining_request.draining_id=d.id INNER JOIN slot ON draining_request.slot_id=slot.id INNER JOIN home ON d.user_id=home.user_id INNER JOIN user ON d.user_id=user.id  WHERE draining_request.accepted = 1 AND d.done_draining=0 ORDER BY session_date';
    ...
    async getDrainingAccepted() {
        return this.db.query(this.GET_DRAINING_ACCEPTED_UNDONE);
    }
